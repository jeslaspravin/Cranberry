...
void parseFailed(CXCursor cursor, SourceGeneratorContext *srcGenContext, Logger::SourceLocationType srcLoc, FmtType &&fmtMsg, Args &&...args)
{
    // Just push and pop debug here to enable log level
    SCOPED_MUTE_LOG_SEVERITIES(Logger::Debug);
    LOG_ERROR(
        "SourceGenerator", "{} error ParseFailed: Reflection parsing failed - {}", clang_getCursorLocation(cursor),
...
void parseEnumsVisitor(CXCursor cursor, SourceGeneratorContext *srcGenContext)
{
    ...
    if (!bMetaParsed)
    {
        parseFailed(cursor, srcGenContext, LOG_CURR_SRC_LOC(), TCHAR("Error when parsing meta annotations"));
        return;
    }
    ...
    clang_visitChildren(
        ...
        [](CXCursor c, CXCursor /*p*/, CXClientData clientData)
        {
            ...
            case CXCursor_EnumConstantDecl:
            {
                ...
                if (!bMetaParsed)
                {
                    parseFailed(c, localCntxt->srcGenContext, LOG_CURR_SRC_LOC(), TCHAR("Error when parsing meta annotations"));
...
void parseMemberFieldVisitor(CXCursor cursor, LocalContext &localCntxt)
{
    ...
    {
        parseFailed(cursor, localCntxt.srcGenContext, LOG_CURR_SRC_LOC(), TCHAR("Invalid member field {}"), fieldName);
        return;
    }
    ...
    if (!bMetaParsed)
    {
        parseFailed(cursor, localCntxt.srcGenContext, LOG_CURR_SRC_LOC(), TCHAR("Error when parsing meta annotations"));
        return;
    }
...
void parseMemberCppMethodsVisitor(CXCursor cursor, LocalContext &localCntxt)
{
    ...
    {
        parseFailed(cursor, localCntxt.srcGenContext, LOG_CURR_SRC_LOC(), TCHAR("Invalid function {}"), funcName);
        return;
    }
    ...
    {
        parseFailed(
            cursor, localCntxt.srcGenContext, LOG_CURR_SRC_LOC(),
...
void parseClassMemberVisitor(CXCursor cursor, LocalContext &localCntxt)
{
    ...
    case CXCursor_CXXBaseSpecifier:
    {
        ...
        {
            parseFailed(cursor, localCntxt.srcGenContext, LOG_CURR_SRC_LOC(), TCHAR("Cannot find declaration of base class {}"), cursorName);
        }
...
void parseClassesVisitor(CXCursor cursor, SourceGeneratorContext *srcGenContext)
{
    ...
    {
        parseFailed(
            cursor, srcGenContext, LOG_CURR_SRC_LOC(),
...
void generatePrereqTypes(CXType type, SourceGeneratorContext *srcGenContext)
{
    ...
    else // Anything other than custom types needs declaration available to be acceptable reflected type
    {
        ...
        {
            parseFailed(
                typeDecl, srcGenContext, LOG_CURR_SRC_LOC(),
