...
void EngineInputCoreModule::createdNewWindow(GenericAppWindow *window) const { inputSystem.registerWindow(window); }
...
void EngineInputCoreModule::init()
{
    createdWindowHandle = IApplicationModule::get()->registerOnWindowCreated(
        AppWindowDelegate::SingleCastDelegateType::createObject(this, &EngineInputCoreModule::createdNewWindow)
    );
}
