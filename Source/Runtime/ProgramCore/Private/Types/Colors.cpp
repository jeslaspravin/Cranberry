...
Color Color::toSrgb() const
{
    ...
    return Color(
        ...
        uint8(Math::round(255 * (value.b > 0.0031308f ? (1.055f * Math::pow(value.b, 1 / 2.4f)) - 0.055f : value.b * 12.92f))),
        colorValue.a
    );
}
...
Color Color::toLinear() const
{
    ...
    return Color(
        ...
        uint8(Math::round(255 * (value.b > 0.04045f ? Math::pow((value.b + 0.055f) / 1.055f, 2.4f) : value.b / 12.92f))),
        colorValue.a
    );
}
...
Color Color::fromHsl(const Vector3D& hsl)
{
    return Color(LinearColor::fromHsl(hsl));
}

LinearColor Color::fromHsv(const Vector3D& hsv)
{
    return Color(LinearColor::fromHsv(hsv));
}

Vector3D Color::toHsv() const
{
    return LinearColor(*this).toHsv();
}

Vector3D Color::toHsl() const
{
    return LinearColor(*this).toHsl();
}
...
// http://en.wikipedia.org/wiki/HSL_color_space
LinearColor LinearColor::fromHsv(const Vector3D& hsv)
{
    const float& h = hsv[0];
    const float& s = hsv[1];
    const float& v = hsv[2];
...
Color ColorConst::random()
{
    return Color(LinearColorConst::random());
}

LinearColor LinearColorConst::random()
{
    // use golden ratio
