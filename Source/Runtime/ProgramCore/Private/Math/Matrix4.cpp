...
Matrix4::Matrix4(const Vector3D &c1, const Vector3D &c2, const Vector3D &c3, const Vector3D &c4,
    const float &c4w /*=1.0f*/)
    : value(c1.x(), c1.y(), c1.z(), 0, c2.x(), c2.y(), c2.z(), 0, c3.x(), c3.y(), c3.z(), 0, c4.x(),
        c4.y(), c4.z(), c4w)
{}
...
Matrix4::Matrix4(const Vector4D &c1, const Vector4D &c2, const Vector4D &c3, const Vector4D &c4)
    : value(c1.x(), c1.y(), c1.z(), c1.w(), c2.x(), c2.y(), c2.z(), c2.w(), c3.x(), c3.y(), c3.z(),
        c3.w(), c4.x(), c4.y(), c4.z(), c4.w())
{}

Matrix4::Matrix4(const float &c1x, const float &c1y, const float &c1z, const float &c1w,
    const float &c2x, const float &c2y, const float &c2z, const float &c2w, const float &c3x,
    const float &c3y, const float &c3z, const float &c3w, const float &c4x, const float &c4y,
...
Matrix4 Matrix4::operator*(const float &scalar) const { return value * scalar; }
...
Matrix4 Matrix4::operator/(const float &scalar) const { return Matrix4(value / scalar); }
