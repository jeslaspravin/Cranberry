...
{
    // For reference value alone remove reference and make it as pointer as reference param is just its pointer being passed around
    ...
    // Given a list of types returns offsets of each types if each type data is placed in continuous aligned memory stack, First type will be in lowest memory location and last type in highest
    ...
    public:
        ...
        {
            pushAnArg<Arg>(data + CalculateParamsAlignedLayout<typename TL::AtIndex<RTList, Len - 1 - Index>::TList>::offset, std::forward<Arg>(arg));
        }
    ...
    NODISCARD CONST_EXPR auto pushToStackedData(Args... args)
    {
        ...
        ParamsStackData<
            CalculateParamsAlignedLayout<typename TL::Reverse<TypesL>::type>::value,
            CalculateMaxAlignment<TypesL>::value
        > ds;
        PushPopStackedData<TypesL>
            ::template pushToStackedData<Args...>(ds.vals, std::forward<Args>(args)..., std::make_index_sequence<sizeof...(Args)>{});
        return ds;
    }
    CONST_EXPR ParamsStackData<0> pushToStackedData()
    {
        return {};
    }
