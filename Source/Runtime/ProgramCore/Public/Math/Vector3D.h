...
public:
    ...
    bool isFinite() const;
    Vector3D safeInverse() const;
    Vector3D normalized() const;
    Vector3D safeNormalized(float threshold = SMALL_EPSILON) const;
    float length() const;
...
FORCE_INLINE Vector3D operator/(float n, const Vector3D &d) { return Vector3D(n / d.value); }

FORCE_INLINE Vector3D operator-(float n, const Vector3D &d) { return Vector3D(n - d.value); }

FORCE_INLINE Vector3D operator*(float n, const Vector3D &d) { return d * n; }
