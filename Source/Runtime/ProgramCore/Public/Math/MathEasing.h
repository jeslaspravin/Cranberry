...
public:
    ...
    static Type slerp(const Type& a, const Type& b, float t)
    {
        ...
        // Finding vector(y-axis) perpendicular to 'a'(assuming as x-axis) in plane formed among origin, 'a', 'b' by rejection
        Type yAxis = (b - (a * dotVal)).normalized();
        
        return a * Math::cos(theta) + yAxis * Math::sin(theta);
    }

    // Following are some easing functions, Each function has an index number corresponding to function in graph https://www.desmos.com/calculator/km86swmxft
    // or here https://easings.net/
    // Index : 1
    ...
    {
        if (t == 0.0f || Math::isEqual(t, 1.0f)) return Math::ceil(t);
...
#define BACK_S2 2.5949095f;
    // Index : 18
    FORCE_INLINE static float backIn(float t)
    {
...
#define BOUNCE_6 (2.625f * BOUNCE_1)
    // Index : 20
    FORCE_INLINE static float bounceIn(float t)
    {
        return 1.0f - bounceOut(1 - t);
    }
    // Index : 21
