...
concept IsReflectedFundamental_Internal = std::disjunction_v<FOR_EACH_CORE_TYPES_UNIQUE_FIRST_LAST(IS_TYPE_SAME_FIRST, IS_TYPE_SAME, IS_TYPE_SAME)>;
template <typename Type>
concept IsReflectedSpecial_Internal = std::disjunction_v<FOR_EACH_SPECIAL_TYPES_UNIQUE_FIRST_LAST(IS_TYPE_SAME_FIRST, IS_TYPE_SAME, IS_TYPE_SAME)>;
...
private:
    ...
    {
        // Qualified type cannot have more than 1 inner type as that means double or more reference/pointer combination
        fatalAssert(propInfo.thisProperty->typeInfo->innerType == nullptr || propInfo.thisProperty->typeInfo->innerType->innerType == nullptr
            , "Qualification for property %s is not allowed for field types in field %s", propInfo.thisProperty->nameString, propInfo.fieldProperty->nameString);
...
public:
     ...
     * 
     * In case of custom types like map or vector or pair the values if nested into another custom type has to be handled manually by caller when receiving call back with types
     */
    template <typename Visitable>
