 ...
 * Above note is mandatory if there is diamond inheritance in your hierarchy(Not recommended to have such hierarchy)
 *
 * allocate and deallocate functions are used to separate allocation from construction
 * newObject and deleteObject are used as alloc + construction and delete. In this case canDeallocate must always return false
 * 
...
public:
    ...
    template <typename Type>
    static bool canDeallocate(void* ptr) { return false; }
    template <typename Type>
...
namespace CtorPolicyHelper
{
    ...
    // If incoming pointer is null, Then this function uses newObject from construction policy to create and construct object
