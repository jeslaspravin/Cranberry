...
#define FOR_EACH_SPECIAL_TYPES_UNIQUE_FIRST_LAST(FirstMacroName, MacroName, LastMacroName)              \
    FirstMacroName(String) MacroName(StringID) MacroName(Color) MacroName(LinearColor)                  \
        MacroName(Vector2D) MacroName(Vector3D) MacroName(Vector4D) MacroName(Matrix2)                  \
            MacroName(Matrix3) MacroName(Matrix4) MacroName(Rotation) LastMacroName(Transform3D)

#define FOR_EACH_SPECIAL_TYPES(MacroName)                                                               \
    FOR_EACH_SPECIAL_TYPES_UNIQUE_FIRST_LAST(MacroName, MacroName, MacroName)

#define FOR_EACH_MAP_SET_TYPES(FirstMacroName, MacroName, LastMacroName)                                \
    FirstMacroName(std::map) MacroName(std::unordered_map) MacroName(std::set)                          \
        LastMacroName(std::unordered_set)

#define FOR_EACH_CUSTOM_TYPES_UNIQUE_FIRST_LAST(FirstMacroName, MacroName, LastMacroName)               \
    FirstMacroName(std::pair) MacroName(std::vector)                                                    \
        FOR_EACH_MAP_SET_TYPES(MacroName, MacroName, LastMacroName)

#define FOR_EACH_CUSTOM_TYPES(MacroName)                                                                \
    FOR_EACH_CUSTOM_TYPES_UNIQUE_FIRST_LAST(MacroName, MacroName, MacroName)

#define CONTAINERTYPE_VALIDATIONS(TypeName, ValueTypeName)                                                                                                                           \
    static_assert(                                                                                                                                                                   \
        std::is_same_v<UnderlyingType<TypeName>,                                                                                                                                     \
            std::vector<                                                                                                                                                             \
                ValueTypeName>> || std::is_same_v<UnderlyingType<TypeName>, std::set<ValueTypeName>> || std::is_same_v<UnderlyingType<TypeName>, std::unordered_set<ValueTypeName>>, \
        "Only basic vector, set and unordered set with default allocator and hash functions are "                                                                                    \
        "supported!")
...
#define MAPTYPE_VALIDATIONS(TypeName, KeyTypeName, ValueTypeName)                                                            \
    static_assert(                                                                                                           \
        std::is_same_v<UnderlyingType<TypeName>,                                                                             \
            std::map<KeyTypeName,                                                                                            \
                ValueTypeName>> || std::is_same_v<UnderlyingType<TypeName>, std::unordered_map<KeyTypeName, ValueTypeName>>, \
...
#define OVERRIDE_CONSTRUCTION_POLICY(PolicyTypeName)                                                    \
    typedef PolicyTypeName CONSTRUCTION_POLICY_TYPEDEF_NAME;
