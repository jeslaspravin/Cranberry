...
#define CONTAINERTYPE_VALIDATIONS(TypeName, ValueTypeName) \
    static_assert(std::is_same_v<UnderlyingType<TypeName>, std::vector<ValueTypeName>> \
...
#define MAPTYPE_VALIDATIONS(TypeName, KeyTypeName, ValueTypeName) \
    static_assert(std::is_same_v<UnderlyingType<TypeName>, std::map<KeyTypeName, ValueTypeName>> \
