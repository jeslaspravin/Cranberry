...
Type getInputFormat(const ReflectFieldType &fieldType)
{
    ...
    static const std::map<uint32, std::set<Type>> ColumnSizeFormatMapper = {
        {1, { Int,     UInt,      Float,      Double, ShortInt, UShortInt, Byte,      UByte,      Int2,    UInt2,     Float2,
 Double2, ShortInt2, UShortInt2, Byte2,  UByte2,   Int3,      UInt3,     Float3,     Double3, ShortInt3, UShortInt3,
 Byte3,   UByte3,    Int4,       UInt4,  Float4,   Double4,   ShortInt4, UShortInt4, Byte4,   UByte4 }              },
        {2,                                                                                                       { Matrix2x2 }},
        {3,                                                                                                       { Matrix3x3 }},
        {4,                                                                                                       { Matrix4x4 }},
    };
    ...
    std::vector<Type>::iterator intersectionsEnd
        = std::set_intersection(typeMatches.cbegin(), typeMatches.cend(), sizeMatches.cbegin(), sizeMatches.cend(), intersections.begin());
    ...
    intersectionsEnd = std::set_intersection(
        sizeAndTypeMatches.cbegin(), sizeAndTypeMatches.cend(), colMatches.cbegin(), colMatches.cend(), intersections.begin()
    );
