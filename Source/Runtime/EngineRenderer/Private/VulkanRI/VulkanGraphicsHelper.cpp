...
#include "VulkanRI/VulkanInternals/VulkanMemoryAllocator.h"
...
uint32 VulkanGraphicsHelper::shaderToPipelineStageFlags(uint32 shaderStageFlags)
{
    ...
    uint32 pipelineStageFlags = 0;
    for (const VkShaderStageFlagBits& shaderStage : shaderStages)
    {
        if ((shaderStageFlags & shaderStage) == shaderStage)
        {
            switch (shaderStage)
            {
...
uint32 VulkanGraphicsHelper::pipelineToShaderStageFlags(uint32 pipelineStageFlags)
{
    ...
    {
        if ((pipelineStageFlags & pipelineStage) == pipelineStage)
        {
