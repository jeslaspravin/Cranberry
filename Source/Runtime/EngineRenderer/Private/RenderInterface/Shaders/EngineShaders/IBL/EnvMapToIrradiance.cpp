...
{
    DECLARE_GRAPHICS_RESOURCE(EnvMapToDiffuseIrradiance, <EXPAND_ARGS(SizeX, SizeY, SizeZ)>,
        ComputeShaderConfigTemplated, <EXPAND_ARGS(SizeX, SizeY, SizeZ)>)

public:
    EnvMapToDiffuseIrradiance()
        : BaseType(ENVMAPTODIFFIRRAD_SHADER_NAME)
    {
        static ComputePipelineFactoryRegistrant ENVMAPTOIRRAD_SHADER_PIPELINE_REGISTER(
...
DEFINE_TEMPLATED_GRAPHICS_RESOURCE(EnvMapToDiffuseIrradiance,
...
{
    DECLARE_GRAPHICS_RESOURCE(HDRIToPrefilteredSpecular, <EXPAND_ARGS(SizeX, SizeY, SizeZ)>,
        ComputeShaderConfigTemplated, <EXPAND_ARGS(SizeX, SizeY, SizeZ)>)
...
public:
    ...
    {
        static ComputePipelineFactoryRegistrant HDRITOPREFILTEREDSPEC_SHADER_PIPELINE_REGISTER(
            this->getResourceName());
    }
        ...
        std::map<String, struct SpecializationConstantEntry> &specializationConst) const final
    {
        ...
        specializationConst[TCHAR("MIP_COUNT")]
            = SpecializationConstUtility::fromValue(EngineSettings::maxPrefilteredCubeMiplevels.get());
    }
};

template HDRIToPrefilteredSpecular<16, 16, 1>;

DEFINE_TEMPLATED_GRAPHICS_RESOURCE(HDRIToPrefilteredSpecular,
    <EXPAND_ARGS(uint32 SizeX, uint32 SizeY, uint32 SizeZ)>, <EXPAND_ARGS(SizeX, SizeY, SizeZ)>)