...
//
// Draws Simple3D colored per instance and uses view and model data from vertex per instance to transform
// vertices
//
...
protected:
    /* UniqueUtilityShader overrides */
    EVertexType::Type vertexUsed() const override { return EVertexType::InstancedSimple3DColor; }
    /* overrides ends */
...
//
// Draws Simple3D colored per vertex, vertices are already transformed to world space and uses view data
// to transform vertices Shader name + Topology + (DWrite if depth writing)
//
template <EPrimitiveTopology::Type Topology, bool DepthWrite>
...
//
// Draws Simple3D colored per instance, vertices are already transformed to world space and uses view
// data to transform vertices and Push constant to determine color per instance
//
