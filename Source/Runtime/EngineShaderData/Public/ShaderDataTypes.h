...
// For texel samplerBuffer, imageBuffer and input attachments(array size will always be 1 for input
// attachments)
struct ReflectTexelBufferShaderField
{
...
struct ReflectDescriptorBody
{
     ...
     *
     * NOTE : Useful in places where all descriptors of a set is used in every stage(none sparse usage
     * like using *.inl to just include all, View or Vertex descriptors is one case) As each descriptors
     * will have same usage. In case of sparse usage of descriptors will have different usage and this
...
struct SpecializationConstantDefaultValue
{
    ...
    SpecializationConstantDefaultValue() { memset(&defaultValue, 0, sizeof(defaultValue)); }
    SpecializationConstantDefaultValue(bool value) { defaultValue.boolVal = value; }
    SpecializationConstantDefaultValue(int value) { defaultValue.i32Val = value; }
    SpecializationConstantDefaultValue(uint32_t value) { defaultValue.u32Val = value; }
    SpecializationConstantDefaultValue(float value) { defaultValue.f32Val = value; }
    SpecializationConstantDefaultValue(double value) { defaultValue.f64Val = value; }
