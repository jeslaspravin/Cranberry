...
public:
    /**
    * IReferenceCollector::clearReferences - Must clear holding references to passed in objects as they will be deleted
    *
...
private:
    ...
    // Maps directly to CBE::ObjectAllocatorBase's allocValidity indices and holds true if an object is referenced, if not referenced it will be detroyed
    // If marked for destroy it will be unreferenced and destroyed
    std::unordered_map<CBEClass, BitArray<uint64>> objUsedFlags;
    // In collecting state classes that left to be crawled and collected, In clearing stage classes that are not cleared yet
...
public:
    /**
    * CoreObjectGC::collect Garbage collects objects and stores a context for next collection if collection exceeds time budget
    *
    * Access: public  
    *
