...
void ImageUtils::calcHistogramRGB(float *outHistogramR, float *outHistogramG, float *outHistogramB,
    uint32 binCount, const uint8 *texels, uint32 sizeX, uint32 sizeY, uint32 channelNum)
{
    ...
    for (uint32 i = 0; i < pixelsCount; ++i)
    {
        ...
        Vector3D binIndices = Math::floor(Vector3D(texels[pixelStart] / perBinDelta,
            texels[pixelStart + 1] / perBinDelta, texels[pixelStart + 2] / perBinDelta));
