...
{
    reinterpret_cast<ImGuiManager *>(userData)->clipboard
        = TCHAR_TO_UTF8(PlatformFunctions::getClipboard().getChar());
...
ShaderParametersRef ImGuiManager::createTextureParam(const TextureBase *texture,
    ...
    const class LocalPipelineContext &pipelineContext)
{
    if (parentGuiManager)
    {
        return parentGuiManager->createTextureParam(
            texture, graphicsInstance, graphicsHelper, pipelineContext);
    }
    else
    {
        ShaderParametersRef params = graphicsHelper->createShaderParameters(
...
ShaderParametersRef ImGuiManager::findFreeTextureParam(const TextureBase *textureUsed)
{
    if (!freeTextureParams.empty())
    {
        ...
        retVal->setTextureParam(
            TEXTURE_PARAM_NAME, textureUsed->getTextureResource(), getTextureSampler());
        return retVal;
    }
    return nullptr;
}
...
void ImGuiManager::setupInputs()
{
    ...
    {
        inputSystem
            = static_cast<EngineInputCoreModule *>(inputModuleWeak.lock().get())->getInputSystem();
    }
}
...
void ImGuiManager::updateInputs()
{
    ...
    io.MousePos = (Vector2D(inputSystem->analogState(AnalogStates::AbsMouseX)->currentValue,
                       inputSystem->analogState(AnalogStates::AbsMouseY)->currentValue)
                      - windowArea.minBound)
                  * dpiScaleFactor;
    ...
    if (drawData && drawData->Valid)
    {
        ...
        for (int32 cmdListIdx = 0; cmdListIdx < drawData->CmdListsCount; ++cmdListIdx)
        {
            ...
            for (int32 cmdIdx = 0; cmdIdx < uiCmdList->CmdBuffer.Size; ++cmdIdx)
            {
                ...
                {
                    ShaderParametersRef perDrawTexture
                        = getTextureParam(static_cast<const TextureBase *>(drawCmd.TextureId));
                    ...
                    {
                        perDrawTexture
                            = findFreeTextureParam(static_cast<const TextureBase *>(drawCmd.TextureId));
                        if (perDrawTexture.isValid())
                        {
                            texturesUsed.insert(perDrawTexture.reference());
                        }
...
void ImGuiManager::updateRenderResources(class IRenderCommandList *cmdList,
    IGraphicsInstance *graphicsInstance, const GraphicsHelperAPI *graphicsHelper,
    const ImGuiDrawingContext &drawingContext, const class LocalPipelineContext &pipelineContext)
{
    ...
    // Setting up vertex and index buffers
    {
        if (!vertexBuffer.isValid() || !idxBuffer.isValid())
        {
            ...
            WindowCanvasRef windowCanvas
                = IApplicationModule::get()->getWindowManager()->getWindowCanvas(
                    IApplicationModule::get()->mainWindow());
            ...
            {
                BufferResourceRef vertexBuf = graphicsHelper->createReadOnlyVertexBuffer(
                    graphicsInstance, int32(sizeof(ImDrawVert)), 1);
                vertexBuf->setAsStagingResource(true);
                vertexBuf->setResourceName(TCHAR("ImGuiVertices_") + String::toString(i));
                BufferResourceRef idxBuf = graphicsHelper->createReadOnlyIndexBuffer(
                    graphicsInstance, int32(sizeof(ImDrawIdx)), 1);
    ...
    if (!getFontAtlasParam().isValid() && parentGuiManager == nullptr)
    {
        ...
        imguiFontAtlasParams = graphicsHelper->createShaderParameters(
            graphicsInstance, pipelineContext.getPipeline()->getParamLayoutAtSet(0), { 0 });
        imguiFontAtlasParams->setTextureParam(
            TEXTURE_PARAM_NAME, getFontTextureAtlas()->getTextureResource(), getTextureSampler());
        imguiFontAtlasParams->setResourceName(
            TCHAR("ShaderParams_") + getFontTextureAtlas()->getTextureName());
        imguiFontAtlasParams->init();
    }
    if (!imguiTransformParams.isValid())
    {
        imguiTransformParams = graphicsHelper->createShaderParameters(
            graphicsInstance, pipelineContext.getPipeline()->getParamLayoutAtSet(0), { 1 });
    ...
    {
        texturesUsed.insert(
            createTextureParam(texture, graphicsInstance, graphicsHelper, pipelineContext).reference());
    }
...
void ImGuiManager::setupRendering()
{
    ...
    float dpiScaleFactor = IApplicationModule::get()->mainWindow()->dpiScale();
    // Using surface size
    io.DisplaySize = Vector2D(float(EngineSettings::surfaceSize.get().x),
                         float(EngineSettings::surfaceSize.get().y))
                     * dpiScaleFactor;
    ...
    else
    {
        ...
        (
            ...
            {
                SamplerCreateInfo createInfo{ .filtering = ESamplerFiltering::Linear,
                    .mipFiltering = ESamplerFiltering::Linear,
                    .tilingMode = { ESamplerTilingMode::EdgeClamp, ESamplerTilingMode::EdgeClamp,
                        ESamplerTilingMode::EdgeClamp },
                    .mipLodRange = { 0, 0 },
...
void ImGuiManager::releaseRendering()
{
    ...
    (
            ...
            const GraphicsHelperAPI *graphicsHelper)
        {
            ...
            if (!parentGuiManager)
            {
                for (std::pair<TextureBase const *const, ShaderParametersRef> &textureParam :
                    textureParams)
                {
...
void ImGuiManager::draw(class IRenderCommandList *cmdList, IGraphicsInstance *graphicsInstance,
    const GraphicsHelperAPI *graphicsHelper, const ImGuiDrawingContext &drawingContext)
{
    setCurrentContexts();
    ImDrawData *drawData = ImGui::GetDrawData();
    if (!drawData || drawingContext.rtTextures.empty() || drawData->CmdListsCount == 0
    ...
    IRenderInterfaceModule::get()->getRenderManager()->preparePipelineContext(
        &pipelineContext, drawingContext.rtTextures);
    ...
    viewport.maxBound
        = static_cast<RenderTargetTexture *>(drawingContext.rtTextures[0])->getTextureSize();

    Vector2D uiToFbDispScale = Vector2D(float(viewport.maxBound.x), float(viewport.maxBound.y))
                               / Vector2D(drawData->DisplaySize);
    ...
    cmdList->cmdBeginRenderPass(
        drawingContext.cmdBuffer, pipelineContext, viewport, additionalProps, clearVal);
    {
        ...
        cmdList->cmdBindDescriptorsSets(
            drawingContext.cmdBuffer, pipelineContext, imguiTransformParams.reference());
            ...
            for (int32 cmdIdx = 0; cmdIdx < uiCmdList->CmdBuffer.Size; ++cmdIdx)
            {
                ...
                if (scissor.intersect(viewport))
                {
                    ...
                    if (drawCmd.TextureId)
                    {
                        perDrawTexture
                            = getTextureParam(static_cast<const TextureBase *>(drawCmd.TextureId));
                        fatalAssert(perDrawTexture.isValid(),
                            "%s(): Failed getting texture parameters for imgui", __func__);
                    }
                    cmdList->cmdBindDescriptorsSets(
                        drawingContext.cmdBuffer, pipelineContext, perDrawTexture.reference());
                    cmdList->cmdSetViewportAndScissor(drawingContext.cmdBuffer, viewport, scissor);
                    cmdList->cmdDrawIndexed(drawingContext.cmdBuffer, idxOffset + drawCmd.IdxOffset,
                        drawCmd.ElemCount, 0, 1, vertOffset + drawCmd.VtxOffset);
                }
            }
...
void ImGuiManager::addFont(const String &fontAssetPath, float fontSize)
{
    ...
    else
    {
        ...
        {
            imguiFontAtlasParams->setTextureParam(
                TCHAR("textureAtlas"), getFontTextureAtlas()->getTextureResource(), getTextureSampler());
        }
    }
}
